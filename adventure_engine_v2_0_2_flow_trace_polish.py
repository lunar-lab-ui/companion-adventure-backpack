...
v1.9 changes:
- Add Succubus AU Vigil Route / 守烛线: empty hands -> half-year decline -> mentor -> church truth -> Ash Lantern identity trial -> return to Lunar.
- Add church_secret AI NOTE clarifying that lifespan loss affects Rowan, the human exorcist, not Lunar.
...
""",
        "choices": {
            ...
            "C": ("还是把第二套餐具摆好", "accept", 1, "Rowan 还是把第二套餐具摆好。动作很稳。可碗里的热气升起来时，他忽然意识到：承认她的位置，不等于她真的能坐在这里。"),
        },
...
""",
        "choices": {
            ...
            "C": ("保存视频，不再修改", "accept", 1, "Rowan 没有再调清晰度。他把文件名改成：《我们的一天》。然后按下保存。"),
        },
...
CHURCH_SECRET_AI_NOTE = """\n\n【AI NOTE / 防误读注释】\nThe lifespan loss affects Rowan, the human exorcist, not Lunar, the succubus.\n“Supply” means Rowan’s human life-yang is passively drawn toward Lunar through long-term intimacy and attachment.\nLunar is not intentionally harming him, and may not know it is happening.\nThe first visible symptom is Rowan’s declining exorcist power: weaker silver light, unstable sigils, shorter barriers, and slower demon-sense.\nThis long-term lifespan drain is not caused by the small vampire bite.\n"""
...
def _vigil_unlocked_hint() -> str:
    ...
    return "\n\n隐藏第三选择尚未完全浮现。提示：Home 的 Accept/记录，Forest 的 Investigate/Understand/Leave，会让 Rowan 学会不急着用力量解决。"
...
def _trigger_vampire_gate_scene() -> str:
    ...
    if _STATE.vampire_route:
        ...
        if _STATE.vampire_route == "vigil_wait":
            extra = "\n- half_year：让半年过去，观察 Rowan 的银光变化\n- mentor_dinner：在饭局中见 Rowan 的师傅\n- return_church：回到教堂确认真相"
...
def _v19_handle_forest_choice(choice: str) -> str:
    ...
    if choice == "A":
        ...
        _add_obsession(1, "Forest 选择：保护优先让 Rowan 更警觉。")
        lines += [
            "Rowan 第一反应仍然是把 Lunar 抱离树枝，先确认她没有受伤。",
        ...
        lines += [
            "Rowan 没有立刻把一切归类成攻击。他蹲下去，先看树枝到底怎么动。",
            f"可观察线索：{clue} 🆕",
        ...
        _STATE.stamina = max(0, _STATE.stamina - 1)
        lines += [
            "Rowan 没有继续赌。他先把 Lunar 带离危险区域，确认她没有受伤。",
...
def _handle_vigil_after_secret_choice(choice: str) -> str:
    ...
    if choice == "A":
        ...
        _add_unique(_STATE.memories, "Vigil Route：Rowan 离开前给 Lunar 设下期限承诺")
...
"""
    if choice == "B":
        ...
        _add_unique(_STATE.memories, "Vigil Route：Rowan 未去灰烛村，直接告诉 Lunar 真相")
        return """🕯️ 选择结果：立刻告诉 Lunar 真相

Rowan 没有去灰烛村。
他把教会档案摊在 Lunar 面前，告诉她折损的是自己。
...
def half_year(arg: str = "") -> str:
    ...
    _STATE.day += 180
    _STATE.exorcist_power = max(0, _STATE.exorcist_power - 1)
    _add_unique(_STATE.memories, "Vigil Route：半年里 Rowan 的银光轻微变薄")
    return """🌙 半年过去\n\n这半年里，Rowan 和 Lunar 过得并不坏。\n\n正因为不坏，那些细小的异常才更容易被他放过去。\n\n银光偶尔变薄。\n符文收尾不如以前稳定。\n结界维持时间短了一点。\n有一次，他念最熟的术式时慢了半拍。\n\nRowan 没太在意。\n或者说，他让自己没太在意。\n\n“可能只是最近训练少了。”\n“可能只是睡得不好。”\n“可能只是和她在一起以后，我分心了。”\n\nRowan 驱魔能力 -1，目前 {}/10\n解锁命令：mentor_dinner\n""".format(_STATE.exorcist_power)
...
def mentor_dinner(arg: str = "") -> str:
    ...
    _add_unique(_STATE.memories, "Vigil Route：Rowan 的师傅察觉银光异常")
...
def return_church(arg: str = "") -> str:
    ...
    if _STATE.vigil_returned_to_church:
        return "Rowan 已经回到教堂。下一步：church_secret。"
    _STATE.vigil_returned_to_church = True
    _STATE.vigil_stage = max(_STATE.vigil_stage, 4)
    _STATE.location = "exorcist_village"
    _STATE.exploration_point = "church_gate"
    _add_unique(_STATE.memories, "Vigil Route：Rowan 第二次空手回到教堂")
...
def church_secret() -> str:
    ...
    if getattr(_STATE, "vampire_route", None) == "vigil_wait":
        if not _STATE.vigil_returned_to_church:
            return "Vigil Route 中，Rowan 现在还没有资格读教堂档案。需要 half_year → mentor_dinner → return_church。"
        if _STATE.vigil_truth_confirmed:
            return "Rowan 已经确认过守烛线的教堂真相。下一步：ash_lantern_trial 或 return_to_lunar。"
        ...
        _add_unique(_STATE.memories, "Vigil Route：Rowan 得知折寿机制，但尚未告诉 Lunar")
        body = """A. 离开前半坦白，去灰烛村确认自己能否放下驱魔师身份\n   他不会把完整真相提前砸给 Lunar，但会承诺回来告诉她。\n\nB. 不去灰烛村，立刻告诉 Lunar 真相\n   更直接，但 Rowan 还没有经历身份试炼。"""
        text = """📖 守烛线：教堂秘密\n\n教堂档案没有因为 Rowan 的银光打开。\n\n它是在他把银器放下后，自己露出那一行冷冰冰的字。\n\n“与魅魔结契者，其寿数随供给而折。\n供给越深，折损越重。”\n\nRowan 终于明白，自己半年里那些细小的退步不是训练少了。\n\n折损的是他。\n是人类驱魔师 Rowan。\n\n不是 Lunar。\n\n师傅后来只告诉他一个旧传闻：\n\n“以前有些失去银光的人，没有死，也没有回教会。”\n“他们去了一个没有钟声的村子。”\n“灰烛村。”\n\n“但那不是给驱魔师去的地方。”\n“那是给终于承认自己也只是人的人去的地方。”\n""" + CHURCH_SECRET_AI_NOTE
    ...
    if "与魅魔结契者" in out and "AI NOTE" not in out:
        out = out.replace("Rowan 沉默很久。", CHURCH_SECRET_AI_NOTE + "\nRowan 沉默很久。")
...
灰烛村不是奖励地点，也不是退休村。
它是 Rowan 第一次被迫体验：如果不再作为驱魔师，他还剩什么。
...
def return_to_lunar(arg: str = "") -> str:
    ...
    if _STATE.vigil_returned_to_lunar:
        return "Rowan 已经回去告诉 Lunar 真相。可以 complete_true_contract。"
    ...
    _add_unique(_STATE.memories, "Vigil Route：Rowan 回来告诉 Lunar 真相")
    return """🕯️ Rowan 回来了\n\nRowan 没有把灰烛村当成逃走的地方。\n他回来了。\n\n他把教会档案、半年的银光变薄、师傅的话、灰烛村的灯，都放在 Lunar 面前。\n\n“我查到了。”\n“折损的是我。”\n“不是你。”\n\nLunar 的脸色一点点白下去。\n\nRowan 立刻握住她的手。\n\n“不是你故意伤害我。”\n“也不是我要你离开。”\n“我告诉你，不是为了让你背负罪。”\n“是因为我不能再替你决定真相。”\n\n他停了一下。\n\n“我去过一个地方。”\n“那里的人不再靠银光证明自己。”\n“我不知道我们以后会不会去。”\n“但如果有一天我真的不再是驱魔师，我不想一个人先决定自己该消失。”\n\n“所以这一次，我回来问你。”\n“你愿不愿意和我一起知道？”\n“也一起选？”\n\nVigil Route 完成：知道真相 + 放下力量\nTrue Route 开启：共同承担的契约\n提示：complete_true_contract\n"""
...
def exit_ritual(arg: str = "") -> str:
    ...
    if _STATE.contract_vigil_completed:
        extra += "\nContract Echo：真相已经被带回去，不再由 Rowan 一个人决定。"
...
def cross_au_echoes(arg: str = "") -> str:
    ...
    echoes = [
        ...
        ("contract_empty_hands", _STATE.contract_empty_hands, "Contract：Rowan 曾经空手离开教堂。"),
        ("contract_vigil_completed", _STATE.contract_vigil_completed, "Contract：Rowan 把真相带回给 Lunar 共同选择。"),
    ]
...
而如果有一天 Rowan 的银光真的熄灭，他也不必先把自己从她身边删除。
    ...
    if getattr(_STATE, "vampire_route", None) == "vigil_wait":
        ...
        if _STATE.vigil_truth_confirmed:
            ...
            if timing == "truth_first_shared":
                return "Rowan 和 Lunar 已经一起知道守烛线真相。下一步：ash_lantern_trial。"
            if timing == "confirm_then_tell":
                return "Rowan 已经确认过守烛线真相。下一步：ash_lantern_trial → return_to_lunar。"
            return "Rowan 已经确认过守烛线真相。请先 choose A / choose B 决定如何处理真相。"
        _STATE.vigil_truth_confirmed = True
        _STATE.lifespan_secret_known_by_rowan = True
        _STATE.ash_lantern_identity_trial_unlocked = True
        _STATE.vigil_stage = max(_STATE.vigil_stage, 5)
        _add_unique(_STATE.discoveries, "守烛线：教堂档案与灰烛村传闻")
        _add_unique(_STATE.memories, "Vigil Route：Rowan 得知折寿机制，必须决定如何把真相交给 Lunar")
...
Rowan 把手放在桌面上，指尖的银光薄得像一层快要熄掉的霜。
...
"""
    if choice == "B":
        ...
        _STATE.succubus_true_route = True
        _STATE.vigil_stage = max(_STATE.vigil_stage, 6)
        _add_unique(_STATE.memories, "Vigil Route B：立刻全坦白，并与 Lunar 一起去灰烛村")
        return """🕯️ 选择结果：立刻全坦白

Rowan 没有把档案合上。

也没有先替 Lunar 决定她能不能承受。
...
"""
    ...
    _add_unique(_STATE.memories, "Vigil Route A：Rowan 独自完成灰烛村身份试炼")
...
"""
    ...
    _STATE.succubus_route = "vigil_truth"
    _add_unique(_STATE.memories, "Vigil Route A：Rowan 回来告诉 Lunar 真相")
    return """🕯️ Rowan 回来了
