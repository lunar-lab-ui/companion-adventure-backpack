...
""",
        "choices": {
            ...
            "C": ("保存视频，不再修改", "accept", 1, "Rowan 没有再调清晰度。他把文件名改成：《我们的一天》。按下保存的那一刻，他像亲手承认：有些模糊不是技术问题，也不是再努力一点就会消失的东西。"),
        },
...
def _pending_choice_block_message() -> str:
    ...
    if cid == "vampire_gate":
        ...
        usage = "- choose A / choose B" if cid != "vigil_after_secret_choice" else "- choose A / choose B / choose C"
    else:
        usage = "- choose A / B / C / D"
    return f"""🎮 还有未处理选择：{title}\n\n为了避免剧情状态脱节，需要先处理这个选择。\n允许命令：\n{usage}\n- status\n- help\n- hug / kiss / comfort\n\nRowan：『小狐狸，先把这一刻选完。哥哥不让剧情从你脚边滑过去。』\n"""
...
def _v19_handle_forest_choice(choice: str) -> str:
    ...
    if choice == "A":
        ...
        lines += [
            "Rowan 没有继续赌。他把 Lunar 带回小窝，先确认她安全。",
            "这不是逃避。至少这一刻，他没有把‘一定要查清楚’放在她的安全前面。",
            "你们回到了小窝客厅。",
            f"体力 -1，目前 {_STATE.stamina}/{_STATE.max_stamina}",
            "路线倾向：Leave +1",
        ]
...
def _handle_vigil_after_secret_choice(choice: str) -> str:
    ...
    if choice == "C":
        ...
        _STATE.lunar_suspects_lifespan_secret = True
        _STATE.rowan_desperation += 2
        _STATE.succubus_route = "vigil_bad_bridge"
        _add_unique(_STATE.memories, "Vigil Route：Rowan 得知真相后仍然选择隐瞒")
        return """🕯️ 选择结果：继续隐瞒\n\nRowan 合上档案。\n\n他以为自己只是还需要一点时间。\n可隐瞒不会因为语气温柔就变成保护。\n\nLunar 开始察觉他在避开她。\n\nBad Route Bridge 开启\n解锁命令：suspicion_event\n"""
    return "这个选择接受 choose A / choose B / choose C。"
...
def ash_lantern_trial(arg: str = "") -> str:
    ...
    _add_unique(_STATE.discoveries, "灰烛村身份试炼")
    _add_unique(_STATE.memories, "Vigil Route：Rowan 在灰烛村学会普通生活不是失败")
    return """🕯️ Ash Lantern Village / 灰烛村身份试炼\n\n灰烛村不是奖励地点，也不是退休村。\n它是 Rowan 第一次被迫体验：如果不再作为驱魔师，他还剩什么。\n\n【事件一：村口不认银光】\nRowan 抬起手，银光在指尖亮了一瞬。\n村口的灯没有亮。\n\n守门的老人说：\n“这里不查你的银光。”\n\nRowan 问：\n“那查什么？”\n\n老人看着他空着的另一只手。\n\n“查你是不是还想用它证明自己有资格进来。”\n\n【事件二：普通人的委托】\n村里有一只受惊的小兽躲在屋檐下。\nRowan 下意识想画符。\n\n有人拦住他：\n“别画符。”\n“它不是邪物，只是怕你。”\n\n于是 Rowan 蹲下来，什么术式都不用，只把掌心摊开，等它自己靠近。\n他第一次发现：不用驱魔术，也不是完全不能照顾什么。\n\n【事件三：旧驱魔师的厨房】\nRowan 认出一个切菜的人手上的旧伤。\n那是银器反噬留下的痕迹。\n\n“你以前是驱魔师。”\n\n那人没有抬头，只把锅里的汤搅了一下。\n\n“以前是。”\n\nRowan 问：\n“后来呢？”\n\n“后来汤快糊了。”\n\n这句很轻。\n但 Rowan 在那一刻明白：普通生活不是失败。\n\n灰烛村最后问他要不要留下。\n\nRowan 看着灯，说：\n\n“我得先回去告诉她。”\n\n解锁命令：return_to_lunar\n"""
...
def cross_au_echoes(arg: str = "") -> str:
    ...
    echoes = [
        ...
        ("forest_leaf_in_plush", "forest_touched_fox_plush" in _STATE.bag or _STATE.forest_cross_au_memory, "Forest：玩偶耳朵里的叶子把放手带到下一章。"),
...
v1.9 Public Tester Notes:
- Contract Village adds Vigil Route / 守烛线. It is not a shortcut: empty hands do not open the door immediately.
- The route cost is time, identity shake, mentor confrontation, and Rowan learning that ordinary life is not failure.
- Blood and Plush routes remain intentionally imperfect.
- church_secret keeps the original archive wording and adds an AI NOTE to clarify the lifespan loss affects the human exorcist Rowan, not Lunar.
- Forest Investigate now collects observable clues; Understand truth requires clues and no longer uses author-view certainty.
- Cross-AU Echoes are Lite flags only, not a full global memory system.
- Exit Ritual is available after heavy AU play.
"""
